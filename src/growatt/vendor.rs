//! Growatt as an implementation of the server's [`Vendor`] seam.
//!
//! One type, one `impl`, and the capabilities the server asks for. The knowledge each capability needs
//! lives in its own module — firmware in [`super::firmware`] — so this file stays a map of what is wired
//! up rather than a place where meaning accumulates.
//!
//! `Message<'a>` is a parsed [`Frame`]: the whole protocol is one frame shape (§4 of the specification), so
//! the vendor's own strongly typed value and the unit the cloud sends happen to coincide. A vendor whose
//! transport carried several message shapes would put an enumeration here instead, and nothing on the
//! server's side would change.
...
impl Vendor for Growatt {
    type Message<'a> = Frame;
...
mod tests {
    ...
    fn a_frame_round_trips_through_the_seam() {
        ...
        assert!(Growatt.advertised_firmware(&parsed).is_none());
    }

    #[test]
    fn the_firmware_request_says_what_the_device_says_and_no_more() {
        let firmware = crate::vendor::AdvertisedFirmware {
        ...
        let request = Growatt.firmware_request(&firmware).body(()).expect("a request");
